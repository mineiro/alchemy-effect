...
`--adopt` provides an `AdoptPolicy = true` to the entire deploy.
Resource providers that opt in to adoption (e.g. `Cloudflare.Worker`
custom domains, `Cloudflare.SecretsStore`, `Cloudflare.SecretsStore.Secret`)
will reconcile against pre-existing cloud resources instead of failing
with "already exists" errors.

This is the right escape hatch when you've **lost or wiped your state
store** but the underlying cloud infrastructure is still in place. Run
`alchemy deploy --adopt` once to re-import everything; subsequent
deploys can drop the flag.

Without `--adopt`, conflicts surface loudly so an unintentional
collision with a production resource is never silently taken over.
