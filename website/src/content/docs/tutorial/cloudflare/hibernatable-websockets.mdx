...
## A Socket-based `connect` helper

We want each test to say "give me a connected client" and get back
something with `send` and `next` methods. Build that on top of
...
- `Socket.makeWebSocket(url)` builds a `Socket` resource. It needs
  a `WebSocketConstructor` in context, which we'll provide at the
  test boundary.
- `socket.writer` returns a function for sending frames. Acquiring
  it once is more efficient than re-deriving it per send.
- `socket.runString` consumes inbound text frames and hands each
  one to your callback. We push them into an unbounded `Queue` so
  the test can pull them off in order with `Queue.take`.
- `Effect.forkScoped` runs the read loop in the background, tied
  to the surrounding scope — when the test scope closes, the loop
  is interrupted and the socket is closed.
- The `onOpen` hook fulfils a `Deferred`, and we wait on it before
  returning. That guarantees the caller can `send` immediately
  without racing the handshake.

## Write the test

Open two clients to the same room, have one send `"hello bob"`,
and assert the other receives the broadcast:
...
Two pieces at the bottom do the heavy lifting:

- `Effect.scoped` closes the scope at the end of the test, which
  interrupts every forked read loop and closes both sockets.
- `Effect.provide(Socket.layerWebSocketConstructorGlobal)` supplies
  the runtime's global `WebSocket` constructor so `Socket.makeWebSocket`
  knows how to actually open a connection. On Bun and Node 22+ this
  picks up the built-in `WebSocket`.
